...
# Test für Hinweistext bei erfolgreicher API-Antwort
...
# Test für Hinweistext bei Fehler (z. B. Timeout)
@patch("app.services.api_service.requests.get", side_effect=RequestException("Timeout"))
def test_hole_hinweistext_fallback_on_exception(mock_get, example_reiseziel):
    result = APIService.hole_hinweistext(example_reiseziel)
    assert result == "Hinweis nicht verfügbar."

# Test für Bild-URL bei erfolgreicher API-Antwort mit HTTPS-Link
@patch("app.services.api_service.requests.get")
...
# Test für Bild-URL wenn kein gültiger Link zurückgegeben wird
@patch("app.services.api_service.requests.get")
def test_hole_bild_url_invalid_data_returns_placeholder(mock_get, example_reiseziel):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "thumbnail": {
...
# Test für Bild-URL bei Ausnahme (z. B. Netzwerkausfall)
@patch("app.services.api_service.requests.get", side_effect=RequestException("Network Error"))
def test_hole_bild_url_fallback_on_exception(mock_get, example_reiseziel):
    result = APIService.hole_bild_url(example_reiseziel)
    assert result == APIService.PLACEHOLDER_IMAGE_URL

# Test für statische Platzhalterbild-URL
