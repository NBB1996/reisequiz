...
def dummy_level():
    return Level(
        ...
        antwortanzahl=4,
        verpixelung=15,
        beschreibung="Mittel - Für Reise-Erfahrene")
...
def test_verpixle_bild_success(
        mock_image_open,
        mock_requests_get,
        dummy_level: Level):
    ...
    assert result.startswith(
        "data:image/jpeg;base64,"), "Base64-Bild nicht korrekt generiert"
...
@patch("app.quiz_engine.APIService.hole_bild_url",
       return_value="http://image.jpg")
@patch("app.quiz_engine.LinkGenerator.booking_deeplink",
       return_value="http://booking")
@patch("app.quiz_engine.LinkGenerator.wikipedia_link_generator",
       return_value="http://wiki")
def test_erzeuge_reiseziel_details(
        mock_wiki,
        mock_booking,
        mock_bild,
        mock_text):
...
def test_generiere_quizfrage_valid(
    ...
    mock_lade_reiseziele.return_value = [
        make_reiseziel(
            z["name"],
            z["kategorie"],
            z["kontinent"]) for z in dummy_reiseziele]
...
@patch("app.quiz_engine.lade_reiseziele")
def test_generiere_quizfrage_too_few_options(
        mock_lade_reiseziele,
        dummy_kategorie,
        dummy_kontinent):
